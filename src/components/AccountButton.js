...
export default function AccountButton(props) {
    ...
    return (
        <div className={`account-button ${props.side}-button`} style={(props.side === "right" || !localStorage.getItem("currentUser")) ? { cursor: "pointer" } : {}} onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
            ...
            <div className="inner-account-button" style={{ color: isHovered ? "#ff2d00" : "#ffffff" }}><i className="fas fa-user"></i><p className="account-button-text">{props.side === "left" ? "Criar Conta" : "Entrar"}</p></div>}
        </div>
    );
}